...
<head>
    ...
    <script type="text/javascript">
        function del(questionId){
            ...
            if(ok){
                alert("正在删除数据，请稍后");
                document.location.href="<%= request.getContextPath()%>/question/delete?questionId="+questionId+"&category=2";
            }
        }
    </script>
    <script type="text/javascript">
...
<body>
<div id="a1">
    <h2><font style="color: #6495ED;">~~判断题~~</font></h2>
</div>
<div id="a3">

</div>
<div id="a2">
    <a href="<%=request.getContextPath()%>/addEstimate.jsp"  id="add">添加题目</a>
</div>
<div class="table" id="alldiv">
    <table border="1" style="text-align:center" id="table" cellspacing="0">
        ...
        <tr>
            <td><%=++i %></td>
            <td><%=estimate.getTitle()%></td>
            <td><%=estimate.getAnswer()%></td>
            <td><a style="color:chocolate;cursor:pointer;" onclick="del(<%=estimate.getQuestionId()%>)" id="xiu2">删除</a></td>
        </tr>
        <%
            }
        %>
